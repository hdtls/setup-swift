    ...
    versionSpec => {
      it.each(['x64', 'arm64'])('on darwin %s', async arch => {
        ...
        const expected = {
          ...
          files: [
            {
              filename: 'swift-5.8-RELEASE-osx.pkg',
              platform: 'darwin',
      ...
      it.each(['x64', 'arm64'])('on ubuntu %s', async arch => {
        ...
        const expected = {
          ...
          files: [
            {
              filename: `swift-5.8-RELEASE-ubuntu22.04${
                arch == 'arm64' ? '-aarch64' : ''
              }.tar.gz`,
              platform: 'ubuntu',
              download_url: `https://download.swift.org/swift-5.8-release/ubuntu2204${
                arch == 'arm64' ? '-aarch64' : ''
              }/swift-5.8-RELEASE/swift-5.8-RELEASE-ubuntu22.04${
                arch == 'arm64' ? '-aarch64' : ''
              }.tar.gz`,
              arch: arch,
              platform_version: '22.04'
