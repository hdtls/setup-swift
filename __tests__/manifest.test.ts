  ...
  describe.each(['5'])(
    ...
    versionSpec => {
      it.each(['x64', 'arm64'])('on darwin %s', async arch => {
        const actual = await mm.resolve(versionSpec, 'darwin', arch);
        const expected = {
          version: 'swift-5.9.1-RELEASE',
          stable: true,
          release_url: '',
          files: [
            {
              filename: 'swift-5.9.1-RELEASE-osx.pkg',
              platform: 'darwin',
              download_url: `https://download.swift.org/swift-5.9.1-release/xcode/swift-5.9.1-RELEASE/swift-5.9.1-RELEASE-osx.pkg`,
              arch: arch,
              platform_version: undefined
            }
          ]
      ...
      it.each(['x64', 'arm64'])('on ubuntu %s', async arch => {
        ...
        const expected = {
          ...
          files: [
            {
              filename: `swift-5.9.1-RELEASE-ubuntu22.04${
                arch == 'arm64' ? '-aarch64' : ''
              }.tar.gz`,
              platform: 'ubuntu',
              download_url: `https://download.swift.org/swift-5.9.1-release/ubuntu2204${
                arch == 'arm64' ? '-aarch64' : ''
              }/swift-5.9.1-RELEASE/swift-5.9.1-RELEASE-ubuntu22.04${
                arch == 'arm64' ? '-aarch64' : ''
              }.tar.gz`,
              arch: arch,
              platform_version: '22.04'
